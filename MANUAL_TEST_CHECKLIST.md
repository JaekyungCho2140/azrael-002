...
- [√] 브라우저에서 https://id.atlassian.com/manage-profile/security/api-tokens 접속
- [√] **[Create API token]** 클릭
- [√] Label: "Azrael" 입력
- [√] **[Create]** 클릭
- [√] 토큰 복사 (ghp_로 시작하는 긴 문자열)

### 1.2. Azrael 접속 및 로그인
- [√] https://azrael-002.vercel.app 접속
- [√] Google OAuth 로그인 (jkcho@wemade.com)
- [√] 로그인 성공 확인 (메인 화면 표시)

### 1.3. JIRA 연동 탭 확인
- [√] **⚙️ 설정** 버튼 클릭
- [√] **JIRA 연동** 탭 클릭
- [√] JIRA API Token 입력 필드 확인
- [√] [👁️ 표시/숨김] 버튼 동작 확인
...
- [X] 편집 모드 활성화 확인 (파란 테두리) -> 활성화는 되나 주황색 두줄 테두리임.
...
-> 하위 일감 생성 시 T2, T3 테이블 표시를 체크해도, T3 테이블에 하위 일감이 출력되지 않음.
---
...
- [√] 버튼 활성화 상태 확인 (비활성화되지 않음) -> 비활성화 상태
- [v] 마우스 오버 → 툴팁 없음 (활성화됨)
...
- [v] **통계 확인**: "총 1 Epic, X Tasks, Y Subtasks"

### 4.4. JIRA 생성 실행
- [v] **[JIRA 생성]** 버튼 클릭
- [v] ⏳ 로딩 표시 확인 (3-5초)
- [X] ✅ 성공 메시지 확인: -> JIRA 생성 실패: Edge Function 호출 실패 (500) 오류 발생
  - "JIRA 일감이 생성되었습니다!"
